...
_APP_ID = "br.com.biglinux.kernelmanager.udev"
_DOMAIN = "big-kernel-manager"
...
def _blacklist_path() -> str:
    ...
    return os.path.join(config_dir, "big-kernel-manager", "ignored_devices.json")
...
class DriverDialog(Adw.Application):
    ...
    def _on_install(self, _btn: Gtk.Button) -> None:
        self._maybe_blacklist()
        # Launch big-kernel-manager in the user session scope so it survives
