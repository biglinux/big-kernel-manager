...
class BaseManager:
        ...
        try:
            ...
            if process.stdout:
                for line in iter(process.stdout.readline, ""):
                    ...
                    if self._cancelled:
                        self._output(
                            output_callback, _("⚠️ Operation cancelled by user.")
                        )
                        break
            ...
            if self._cancelled:
                ...
                self._output(
                    output_callback,
                    _("❌ {} failed (exit code: {})").format(
                        operation_name, process.returncode
                    ),
                )
                if complete_callback:
                    complete_callback(success)
    ...
    def _progress(
        self, callback: Optional[Callable], fraction: float, text: Optional[str]
    ) -> None:
        """
