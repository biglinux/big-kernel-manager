...
class KernelManagerWindow(Adw.ApplicationWindow):
    ...
    def _build_ui(self):
        ...
        self.stack.add_titled_with_icon(
            self.kernel_page, "kernel", _("Kernel"), "system-software-install-symbolic"
        )
        self.stack.add_titled_with_icon(
            self.mesa_page, "mesa", _("Video Drivers"), "video-display-symbolic"
        )

        # Create view switcher in header bar
    ...
    def _on_about_activated(self, action, param):
        ...
        from core.constants import APP_NAME, APP_VERSION, APP_AUTHOR, APP_WEBSITE
    ...
    def _check_show_warning_dialog(self) -> bool:
        ...
        try:
            show_warning = self.settings_manager.get(
    ...
    def show_warning_dialog(self):
        ...
        dialog = Adw.Dialog()
        dialog.set_title(_("Kernel and Mesa Management"))
    ...
    def _create_warning_content(self) -> Gtk.Box:
        ...
        kernel_items = [
            (
                ...
                "warning",
                "<b>"
                + _("Always keep at least one working kernel")
                + "</b> "
                + _("installed as a fallback"),
            ),
            (
                "emblem-ok-symbolic",
                "success",
        ...
        advice_label.set_markup(
            "<i>"
            + _(
                "If unsure, the <b>LTS kernel</b> and <b>Stable Mesa</b> drivers "
                "are the safest choices for most users."
            )
            + "</i>"
        )
    ...
    def _create_info_card(
        self, title, icon_name, icon_color, badge_text, badge_color, items
    ):
    ...
    def _create_warning_controls(self, dialog) -> Gtk.Box:
        ...
        show_on_startup_switch.set_tooltip_text(_("Show this warning on startup"))
        show_on_startup_switch.connect(
            "notify::active", self._on_warning_switch_toggled
        )
