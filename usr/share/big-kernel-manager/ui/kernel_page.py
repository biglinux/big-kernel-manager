...
class KernelPage(BasePage):
    ...
    def _update_kernel_list(self, kernels, running_kernel_package=""):
        ...
        if installed_kernels:
            header = self._create_section_header(
                _("Installed Kernels"), len(installed_kernels)
            )
        ...
        if available_kernels:
            header = self._create_section_header(
                _("Available Kernels"), len(available_kernels)
            )
            self.kernel_listbox.append(header)
    ...
    def _create_kernel_row(self, kernel):
        ...
        if is_installed:
            ...
            remove_button.set_tooltip_text(_("Remove kernel {}").format(kernel["name"]))

            # Disable remove button for the running kernel (safety protection)
            if is_running:
                remove_button.set_sensitive(False)
                remove_button.set_tooltip_text(
                    _("Cannot remove the currently running kernel")
                )
    ...
    def _on_install_clicked(self, button, kernel):
        ...
        dialog.set_body(
            _(
                "Are you sure you want to install the <b>{}</b> kernel?\n\n"
    ...
    def _on_install_dialog_response(self, dialog, response, kernel, button):
        ...
        self.progress_dialog.append_terminal_output(
            _("Starting installation of {} kernel...").format(kernel["name"])
        )
        self.progress_dialog.append_terminal_output(
    ...
    def _on_remove_clicked(self, button, kernel):
        ...
        if kernel["name"] == self._running_kernel_package:
            self._show_completion_dialog(
                ...
                _(
                    "The kernel you are trying to remove is currently in use.\n"
        ...
        dialog.set_body(
            _(
                "Are you sure you want to remove the <b>{}</b> kernel?\n\n"
    ...
    def _on_remove_dialog_response(self, dialog, response, kernel, button):
        ...
        # Add initial terminal output
        self.progress_dialog.append_terminal_output(
            _("Starting removal of {} kernel...").format(kernel["name"])
        )
        self.progress_dialog.append_terminal_output(
            _("This may take a few minutes. Please wait...")
        )
